...
2026-09-24 复核（抓 `forumdisplay.php?fid=2|6`，桌面 UA、无 Cookie）：两者都返回
**7KB 的「您还未登录」页**，页内没有 `div.threadtype` —— 与上表一致。

### 主题分类（`typeid`）对照

分类是**发帖时贴给主题的标签**（列表里显示成 `[心得技巧]` 前缀），**每个板块各不相同**。
分类列表挂在板块页的 `div.threadtype` 里（链接形如 `forumdisplay.php?fid=14&filter=type&typeid=10`）。

| 板块 | 分类行（实测 / 用户提供） |
|---|---|
| **fid=14**（Windows Mobile / HPC，✅ 游客可读） | 心得技巧#10 / 绿色汉化#11 / 游戏#12 / 多媒体#13 / 数据库#14 / 实用工具#15 / ROM#16 / 硬件报告#17 / HPC#44 / 求助#18 / 站务#19（共 11 个，**本地已用夹具验证**） |
| **fid=2**（Discovery，🚫 登录门） | 全部 / 聚会 / 汽车 / 大杂烩 / 助学 / Discovery / 投资 / 职场 / 文艺 / 版喃 / 显摆 / 晒物劝败 / 装修 / 推片 / YY / 站务（用户提供，**本地无法验证**） |
| **fid=6**（Buy & Sell，🚫 登录门） | 全部 / 手机 / 掌上电脑 / 笔记本电脑 / 无线产品 / 数码相机、摄像机 / MP3随身听 / 各类配件 / 其他好玩的 / 站务（用户提供，**本地无法验证**） |

- 行首的「**全部**」是站点「不筛选」的复位项（链接不带 `typeid`），**不作为发帖分类** ──
  `BoardFilterParser.categories` 只认带 `typeid=` 的链接，会自动排除。
- ⇒ 客户端**不硬编码这份表**：发帖时按当前板块现抓现解析（`ForumRepository.categories(fid:)`）。
  登录门板块只有在**登录态**下才拿得到，所以「读不到」与「没有分类」在界面上必须分开呈现。
