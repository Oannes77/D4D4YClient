...
## 一、操作栏图标分别是什么（提问，顺带留档）

用户问的「分别是什么功能」= 详情页楼层操作栏的图标。它们**只有图标没有文字**，
...
⚠️ 原来还有第六项 `safari`（圆圈指南针）=「网页版」。**当天就被用户判为「没用的功能」并要求取消**
—— 见本文 §七。顺带一提，这个图标在细线单色下本来就容易被看成「禁止 / 拉黑」（`nosign`）。
...
## 六、验收（Codemagic build #53 / commit `2846277`，2026-09-24 18:30）

这个批次起 `runinfo.txt` 生效，产物自己报了出身 —— 一眼对上版本，不用再靠时间戳反推：

```
commit=28462775c7d12c0c3e18fcc72c071d65b6202ec1
branch=main
build=53
...
| 圈注项 | 验收结论 |
|---|---|
| ① 六个图标分别是什么 | 提问，已把「图标 → 功能」对照留档（本文 §一） |
| ② 收藏星标对齐 | ✅ 亮 / 暗两侧星标都与「›」在同一条垂直中线（原来低一整行） |
| ③ 详情多图一张一行 | ✅ `threadImages` 亮 / 暗都是整幅宽图竖排，不再有方块墙 |
| ④ 首页发帖入口 + 取消 FAB | ✅ 亮 / 暗首页：搜索栏收起、「发新帖…」入口常驻、**FAB 已消失**。连带效果：第 2 条（Kepler 那帖）的标题与摘要**完整可见** —— 正是上一轮被 FAB 盖住的那部分 |

其余界面（`thread` / `threadShareMenu` / `userCard` / `newPost` / `replyPlaceholder` /
`reportChat` / `myFollows` / `savedThreads`）无回归；20 张全部通过「有没有渲染」的自动检查
（亮度 1%/99% 分位极差，最低也有 95）。

**仍需人工确认的一条**：搜索栏「下拉才展开」是手势动效，静态截图拍不到 ——
本轮两张首页图都是**收起态**（即预期形态），展开态要在真机 / 模拟器上滑一下确认。

## 七、取消操作栏第六项「网页版」

用户判定：**没用的功能，取消。**

### 为什么它确实没用（复核过，不是听用户的就算了）
「网页版」当初存在的唯一理由是 Sprint 12 写下的那句「站内评分 / 举报等只在论坛网页端提供」。
但复核后：**该站点两套模板（WAP / PC）都没有评分功能**（`FeatureStatus.md` 模块 17，
口径比 Sprint 12 更硬），而**举报客户端已经自己做完了**（复制链接 + 私信管理员）。
⇒ 它指向的收益根本不存在，留着只是一个「点了也不知道要干嘛」的图标。

### 改了什么
- 删掉 `PostDetailRow.actionBar` 里的 `Link(destination: threadURL) { Image("safari") }`
  及其 `post-web` 无障碍标识（没有测试或文档依赖它）。
- 操作栏 **六 → 五**：回复 / 分享 / 收藏 / 关注 / 举报。
- **`threadURL` 属性保留** —— 系统分享与举报仍要用它，只是不再单独做一个浏览器入口。
- 文档同步：`Phase4-Screens.md`（UI 硬约定·操作栏）、`FeatureStatus.md`（操作栏五键）、
  `Sprint18-Changelog.md`（该表里的「六键」加更正指引）。

### ⚠️ 另一个「网页版」不要一起删
`UnavailableFeatureView` / `MySpaceListView` 里也写着「在浏览器中打开论坛网页版」，
但那是**完全不同的东西**：客户端读不出某个服务器列表（未登录 / 结构未识别）时给的**真实出口**，
属于「做不到就明说 + 给真去处」的硬要求，**必须保留**。
判据很简单：**入口指向的功能存不存在** —— 评分不存在（删），某个列表读不出来（留出口）。
