...
| 首页搜索 + 发帖 | **同一排、默认整排收起**：下拉回弹才出现 `[🔍 搜索 …] [发帖]`（搜索框占左侧、发帖按钮在右端）。平时首页顶部只有板块胶囊 + 排序条，看不到搜索、也看不到发帖 —— 首屏纵向空间全给内容。⚠️ **发帖入口与搜索同一可见性：想发帖先下拉**（2026-09-24 用户确认） |
| 首页发帖入口形态 | **不用悬浮按钮（FAB）**，也不单独占一行 —— FAB 固定悬浮必然压住列表正文；单独一行则太碍眼。见上一行 |
| 板块排序条 | 板块胶囊下方一条横向胶囊：**只有排序与时间**（时间档在前、热门在后 —— 以页面顺序为准）。内容与选中态全部来自板块页自身链接，客户端不拼参数、不记账；精华/投票/活动不做（解析时跳过） |
| 主题分类的归属 | **分类不在首页**。分类是**发帖时贴给主题的标签**（`typeid`），在发帖页选（板块不同分类不同，运行时从该板块页解析）。首页、列表页都不显示分类筛选 |
| 发帖页分类 | 发帖页「发布到」下方是「分类」下拉；该板块没开分类就整块不显示（不摆空下拉）。换板块会重新解析并**清空已选**（避免提交张冠李戴的 typeid） |
...
| 1b | `home`（`homeSearchCompose`） | 🆕 **下拉后**的顶部那一排：`[🔍 搜索 …] [发帖]`。ⓘ 这一排默认收起，不先 `swipeDown` 根本拍不到 | home-post-open → 下拉 | ✅ |
...
默认只跑**待验收**的 `AppScreenshotTests.targets`（Sprint 20 为 **12 个界面 = 24 张**：home /
**homeSearchCompose**（下拉态）/ thread / threadShareMenu / threadImages / **threadAttachments** /
userCard / newPost / savedThreads / replyPlaceholder / reportChat / myFollows）；
需要全量回归时在 Codemagic 设 `SCREENSHOT_FULL=1`，跑满 23 个界面 = 46 张。
